...
export default function TopicLayout({
  ...
  const searchResults = useMemo(() => {
    ...
    const sectionMatches = activeHeadings
      .filter((heading) => heading.title.toLowerCase().includes(normalizedQuery))
      .map((heading) => ({
        id: heading.id,
        title: heading.title,
  ...
  const outlineGroups = useMemo(() => {
    ...
    let activeGroup: { id: string; title: string; items: Array<{ id: string; title: string }> } | null = null;

    for (const heading of activeHeadings) {
      const headingQuery = heading.title.toLowerCase();
      const matchesQuery = !hasSearchQuery || headingQuery.includes(normalizedQuery);
  ...
  useEffect(() => {
    ...
    const updateActiveOutline = () => {
      ...
      setActiveOutlineId((previous) => previous === currentId ? previous : currentId);
    };
  ...
  const getSectionIcon = (title: string) => {
    ...
    if (value.includes("getting") || value.includes("overview") || value.includes("introduction")) return BookOpenText;
    if (value.includes("variable") || value.includes("data") || value.includes("types")) return Braces;
    if (value.includes("console") || value.includes("terminal")) return TerminalSquare;
    if (value.includes("function") || value.includes("module") || value.includes("async")) return Code2;
    if (value.includes("security") || value.includes("protection") || value.includes("rules")) return ShieldCheck;
    if (value.includes("best practice") || value.includes("tips") || value.includes("patterns")) return Sparkles;
    if (value.includes("workflow") || value.includes("lifecycle") || value.includes("reconciliation")) return Workflow;
    return BookOpenText;
  };
  ...
  return (
    <div className={`${theme.page} min-h-screen font-sans selection:bg-[#C699FF]/25 transition-colors duration-200`}>
      <header className={`sticky top-0 z-40 border-b ${theme.border} ${theme.panel} backdrop-blur-sm`}>
        <div className='mx-auto flex max-w-[1700px] items-center justify-between gap-4 px-4 py-3 md:px-6'>
          <div className='flex items-center gap-3'>
            <div className='flex h-8 w-8 items-center justify-center rounded-md bg-[#0F1115] text-white shadow-sm ring-1 ring-white/10'>
              <span className='text-lg font-black leading-none tracking-[-0.14em]'>R<span className='text-zinc-500'>_</span></span>
            </div>
          ...
          <div className='hidden flex-1 items-center justify-center md:flex'>
            <div className='relative w-full max-w-xl'>
              <div className={`flex w-full items-center gap-3 rounded-full border px-3 py-2 transition-all duration-200 ${theme.border} ${theme.input} ${docSearch ? "border-[#C699FF]/70 shadow-[0_0_0_1px_rgba(198,153,255,0.38)]" : "focus-within:border-[#C699FF]/60"}`}>
                ...
                {docSearch ? (
                  ...
                  <span className='rounded-full border border-current/15 px-1.5 py-0.5 text-[10px] font-medium uppercase opacity-60'>⌘K</span>
                )}
              </div>
                    ...
                    <div className='max-h-[320px] overflow-y-auto p-2'>
                      {searchResults.map((result) => (
                        ...
                        >
                          ...
                          <span className='text-[10px] uppercase tracking-[0.18em] opacity-60'>Go</span>
                        </button>
                      ))}
                    </div>
                  ) : (
      ...
      <div className='mx-auto flex max-w-[1700px]'>
        <aside className={`hidden w-[280px] shrink-0 border-r lg:flex lg:flex-col`}>
            ...
            <div className='space-y-2.5'>
              {topics.map((item) => {
                ...
                return (
                  ...
                  >
                    <span className={`flex h-7 w-7 items-center justify-center rounded-full border ${isActive ? (darkMode ? 'border-white/15 bg-white/5 text-white' : 'border-[#d9d1c7] bg-[#f0ece6] text-[#111111]') : (darkMode ? 'border-white/10 bg-transparent text-zinc-300' : 'border-[#dad2c7] bg-[#f3efe9] text-[#111111]')}`}>
                      <TopicIcon size={15} className='shrink-0' />
                    </span>
                    <span className='truncate text-[15px] font-medium tracking-[-0.02em]'>{item.title || item.id}</span>
                  </button>
                );
              })}
            </div>
        ...
        <aside className='hidden w-[260px] shrink-0 border-l px-5 py-8 lg:block'>
          <div className='sticky top-24'>
            <p className={`mb-4 text-[10px] font-medium uppercase tracking-[0.2em] ${theme.muted}`}>
              On this page
            </p>
