...
export default function ClientCodeBlock({
  ...
  // Pre-calculate skeleton lines to guarantee stable React Fiber node references.
  // This prevents React DevTools hydration crashes ("The children should not have changed")
  const skeletonOverlay = useMemo(() => {
    if (mounted) return null; // Save computation once hydrated
    const safeCode = code || "";
    const numLines = Math.min(safeCode.split('\n').length, 30);
  ...
  return (
    ...
    >
      ...
      <div className="w-full py-4 overflow-x-auto bg-[#F4F4F0] dark:bg-[#101010] transition-colors duration-200">
        {!mounted ? (
          <div key="skeleton-fallback" className="relative w-full">
            <div 
